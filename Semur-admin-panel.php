...
<html lang="en">
  <head>
    ...
    <title>Կառավարման վահանակ</title>
          ...
          <div class="content-side">
            <ul class="nav-main">
              <li class="nav-main-item">
                <?php echo '<a class="nav-main-link active" href="Semur-admin-panel.php?h='.$_GET['h'].'">' ?>
          ...
          <div class="row items-push">
            <div class="col-sm-6 col-xxl-3">
              <div class="block block-rounded d-flex flex-column h-100 mb-0">
                <div class="block-content block-content-full flex-grow-1 d-flex justify-content-between align-items-center">
                  <dl class="mb-0">
                    <dd class="fs-sm fw-medium fs-sm fw-medium text-muted mb-0">Կայքը բացվել է</dd>
                    <?php
                      $inf = $mysql -> query("SELECT * FROM `opening_number`");
                      foreach($inf as $value);

                      echo '<dt class="fs-3 fw-bold">'.$value['id'].' <dd class="fs-sm fw-medium fs-sm fw-medium text-muted mb-0">անգամ</dd></dt>';
                    ?>
                    <dd class="fs-sm fw-medium fs-sm fw-medium text-muted mb-0">Վերջին անգամ բացվել է</dd>
                    <?php
                      $inf = $mysql -> query("SELECT * FROM `opening_number`");
                      foreach($inf as $value);

                      echo '<dd class="fs-sm fw-medium fs-sm fw-medium text-muted mb-0">'.$value['date'].'</dd>';
                    ?>
                  </dl>
            ...
            <div class="col-sm-6 col-xxl-3">
              <div class="block block-rounded d-flex flex-column h-100 mb-0">
                <div class="block-content block-content-full flex-grow-1 d-flex justify-content-between align-items-center">
                  <dl class="mb-0">
                    <dd class="fs-sm fw-medium fs-sm fw-medium text-muted mb-0">Կայքում նկարների քանակը</dd>
                    <?php
                      $inf = $mysql -> query("SELECT * FROM `gallery`");
                      $in = mysqli_num_rows($inf);

                      echo '<dt class="fs-3 fw-bold">'.$in.'</dt>';
                    ?>
                  </dl>
                  <div class="item item-rounded-lg bg-body-light">
            ...
            <div class="col-sm-6 col-xxl-3">
              <div class="block block-rounded d-flex flex-column h-100 mb-0">
                <div class="block-content block-content-full flex-grow-1 d-flex justify-content-between align-items-center">
                  <dl class="mb-0">
                    <dd class="fs-sm fw-medium fs-sm fw-medium text-muted mb-0">Կատեգորիաների քանակը</dd>
                    <?php
                      $inf = $mysql -> query("SELECT * FROM `assortment`");
                      $in = mysqli_num_rows($inf);
            ...
            <div class="col-sm-6 col-xxl-3">
              <div class="block block-rounded d-flex flex-column h-100 mb-0">
                <div class="block-content block-content-full flex-grow-1 d-flex justify-content-between align-items-center">
                  <dl class="mb-0">
                    <dd class="fs-sm fw-medium fs-sm fw-medium text-muted mb-0">Ապրանքների քանակը</dd>
                    <?php
                      $inf = $mysql -> query("SELECT * FROM `items`");
                      $in = mysqli_num_rows($inf);
                  ...
                  <div class="item item-rounded-lg bg-body-light">
                    <i class="fa fa-boxes-stacked fs-3 text-primary"></i>
                  </div>
                </div>
              </div>
            </div>
